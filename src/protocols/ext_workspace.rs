...
//!
//! This is how we map the protocol concepts to the synoik concepts:
//!
...
pub fn refresh(state: &mut State) {
    ...
    let protocol_state = &mut state.synoik.ext_workspace_state;
    ...
    for (mon, _, ws) in state.synoik.layout.workspaces() {
    ...
    protocol_state.workspace_groups.retain(|output, data| {
        if state.synoik.sorted_outputs.contains(output) {
            return true;
        }
    ...
    for (mon, ws_idx, ws) in state.synoik.layout.workspaces() {
    ...
    for output in &state.synoik.sorted_outputs {
