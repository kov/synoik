...
//! sample a dangling descriptor. Per-frame `VkTexture` allocation is a host round trip on Venus —
//! host time and host pool pressure, see `VulkanRenderer::readback_staging_buffer` — so `output` is
