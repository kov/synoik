...
fn set_fullscreen_on_removed_output_does_not_panic() {
    ...
    // Remove the output on the synoik side. Its wl_output global is disabled but not yet
    // destroyed, so the client's wl_output resource is still valid and usable.
    let output = f.synoik_output(2);
    f.synoik().remove_output(&output);

    // Request fullscreen on the now-removed wl_output. synoik must not panic.
    let window = f.client(id).window(&surface);
