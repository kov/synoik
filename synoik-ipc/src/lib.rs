...
//! by one, in order, and to each request it will respond with a single [`Reply`], which is a
//! `Result` wrapping a [`Response`].
//!
//! If you send a [`Request::EventStream`], synoik will *stop* reading subsequent [`Request`]s, and
...
//!
//! 1. Read the socket filesystem path from [`socket::SOCKET_PATH_ENV`] (`$SYNOIK_SOCKET`).
...
//! 3. Synoik will respond with a single line JSON-formatted [`Reply`].
//! 4. You can keep writing [`Request`]s, each on a single line, and read [`Reply`]s, also each on a
//!    separate line.
//! 5. After you request an event stream, synoik will keep responding with JSON-formatted
//!    [`Event`]s, on a single line each.
//!
//! ## Backwards compatibility
//!
//! This crate follows the synoik version. It is **not** API-stable in terms of the Rust semver. In
...
/// Request from client to synoik.
#[derive(Debug, Serialize, Deserialize, Clone)]
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum Request {
    /// Request the version string for the running synoik instance.
    Version,
...
/// Successful response from synoik to client.
#[derive(Debug, Serialize, Deserialize, Clone)]
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum Response {
    /// A request that does not need a response was handled successfully.
    Handled,
    /// The version string for the running synoik instance.
    Version(String),
...
/// Actions that synoik can perform.
// Variants in this enum should match the spelling of the ones in synoik-config. Most, but not all,
// variants from synoik-config should be present here.
#[derive(Serialize, Deserialize, Debug, Clone)]
...
// Variants in this enum should match the spelling of the ones in synoik-config. Most thigs from
// synoik-config should be present here.
#[derive(Serialize, Deserialize, Debug, Clone)]
#[cfg_attr(feature = "clap", derive(clap::Parser))]
#[cfg_attr(feature = "clap", command(subcommand_value_name = "ACTION"))]
#[cfg_attr(feature = "clap", command(subcommand_help_heading = "Actions"))]
...
pub enum OutputAction {
    ...
    Mode {
        ...
        ///
        /// Run `synoik msg outputs` to see the available modes.
        #[cfg_attr(feature = "clap", arg())]
        mode: ModeToSet,
    },
...
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum ModeToSet {
    /// Synoik will pick the mode automatically.
    Automatic,
    /// Specific mode.
...
#[cfg_attr(feature = "json-schema", derive(schemars::JsonSchema))]
pub enum ScaleToSet {
    /// Synoik will pick the scale automatically.
    Automatic,
    /// Specific scale.
    Specific(f64),
}
...
/// some optional properties are present or absent for certain window types may change across synoik
/// releases.
///
...
pub struct WindowLayout {
    ...
    ///
    /// Does not include synoik decorations like borders.
    ///
...
pub struct Workspace {
    ...
    ///
    /// This is the same index you can use for requests like `synoik msg action focus-workspace`.
    ///
