//! Helper for blocking communication over the synoik socket.
...
/// Name of the environment variable containing the synoik IPC socket path.
pub const SOCKET_PATH_ENV: &str = "SYNOIK_SOCKET";

/// Helper for blocking communication over the synoik socket.
///
/// This struct is used to communicate with the synoik IPC server. It handles the socket connection
...
impl Socket {
    ...
    pub fn connect() -> io::Result<Self> {
        let socket_path = env::var_os(SOCKET_PATH_ENV).ok_or_else(|| {
            io::Error::new(
                ...
                format!("{SOCKET_PATH_ENV} is not set, are you running this within synoik?"),
            )
        })?;
        Self::connect_to(socket_path)
    }

    /// Connects to the synoik IPC socket at the given path.
    ...
    /// Sends a request to synoik and returns the response.
    ///
    /// Return values:
    ///
    /// * `Ok(Ok(response))`: successful [`Response`](crate::Response) from synoik
    /// * `Ok(Err(message))`: error message from synoik
    /// * `Err(error)`: error communicating with synoik
    ...
    ///
    /// ```no_run
    /// use synoik_ipc::{Request, Response};
