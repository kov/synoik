...
| Q6 | **network** | status icon(s) | per-device `QuickMenuToggle`s: wired, Wi-Fi (list), modem, BT-tether, VPN — each w/ submenu | `js/ui/status/network.js`; built `panel.js:303-310`; NMToggle `:1381`; Wi-Fi `:1076`; VPN `:1541` | `[nm]` | 🟡 **Wired / Wi-Fi / VPN tiles ✅**, one per device type as GNOME has them — there is no "Network" tile. Model in `src/network_model.rs` (networks grouped by SSID+mode+security, security derived through libnm's `nm_utils_security_valid`, upstream's sort and 8-row cap, the lock-screen filter); ObjectManager watcher in `src/dbus/network_manager.rs` (coalesced rebuilds, a `GetSettings` cache keyed by profile path) which also feeds the panel icon, retiring its fixed-strength placeholder. Wi-Fi body flips `WirelessEnabled` (inert on a hardware kill), arrow opens the network list and scans; Wired activates/deactivates and lists its profiles; VPN rows toggle a profile each. Secret agent ✅ (`src/dbus/network_agent.rs` + `src/network_secret_dialog.rs`). **Left ⬜**: modem and BT-tether tiles (no hardware), the 15 s rescan + spinner, the hotspot tile state, and the captive-portal helper. Live-validated on the gsrs seat 2026-09-03: the Wired card lists the one profile NM offers eth0 (of three saved), the VPN card both NordVPN profiles. This VM has no Wi-Fi hardware, so the list itself is driven by `debug-set-network` — which is also how it is looked at.
...
- **`QuickMenuToggle`** — ✅ **framework landed** (`src/ui/quick_settings.rs`): a menu tile split into
  ...
  `Consumed`; rows are `Spawn`). Consumers: the three **network** tiles, the **Bluetooth** tile, the
  two volume pickers, the brightness card, the power-profile picker and the **power button**. This is the prerequisite for Q6/Q7/Q8's
...
11. ✅ Q4 brightness — **complete**: Q4a backlight subsystem, Q4b manager algebra, Q4c the QS
    ...
    (`docs/fork/osd-media-port.md`) once that milestone had built an OSD subsystem to show it in.
    Nothing about Q4 is live-validated — this VM has no backlight device. Plan file:
...
12. ✅ **R4 a11y menu** — the `ATIndicator` indicator + its ten-switch menu, all gsettings-backed.
    ...
    for — those are now sequenced as their own milestone in `docs/fork/a11y-port.md`, NOT as
