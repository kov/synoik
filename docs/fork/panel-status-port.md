...
| Q6 | **network** | status icon(s) | per-device `QuickMenuToggle`s: wired, Wi-Fi (list), modem, BT-tether, VPN — each w/ submenu | `js/ui/status/network.js`; built `panel.js:303-310`; NMToggle `:1381`; Wi-Fi `:1076`; VPN `:1541` | `[nm]` | 🟡 panel icon ✅; tile is a QuickMenuToggle — arrow opens an in-menu detail card (header + **Network Settings** row); body opens settings. The **secret agent is done** (`src/dbus/network_agent.rs` + `src/network_secret_dialog.rs`): NM secret requests now reach a modal prompt and are remembered in the keyring, which is the half Settings' own `libnma` agent never covered. **In-menu enable/disable, Wi-Fi list, VPN ⬜** (need NM writes and a device/AP model); no SSID label. This VM has no Wi-Fi hardware, so the list will be headless- and fake-verified only, like Q7 |
