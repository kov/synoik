...
// synoik_v_coords becomes v_uv (0..1). Outputs premultiplied color (premultiplied-over blend).
...
void main() {
    ...
    if (pc.sigma < 0.1) {
        ...
        shadow_value = synoik_rounding_alpha(coords_geo, pc.geo_size, pc.corner_radius);
    } else {
    ...
    if (pc.window_geo_size != vec2(0.0, 0.0)) {
        if (0.0 <= coords_window_geo.x && coords_window_geo.x <= pc.window_geo_size.x
            ...
            float alpha = synoik_rounding_alpha(coords_window_geo, pc.window_geo_size, pc.window_corner_radius);
