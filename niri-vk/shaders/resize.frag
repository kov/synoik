...
// (niri_input_to_curr_geo, niri_geo_to_tex_prev, niri_geo_to_tex_next), and niri_progress /
// niri_curr_geo_to_{prev,next}_geo are dead — so they are dropped here. Each used transform is
...
void main() {
    ...
    if (pc.clip_to_geometry == 1.0) {
        if (coords_curr_geo.x < 0.0 || 1.0 < coords_curr_geo.x
            ...
            color = color * niri_rounding_alpha(coords_curr_geo * pc.curr_geo_size, pc.curr_geo_size, pc.corner_radius);
        }
    }
